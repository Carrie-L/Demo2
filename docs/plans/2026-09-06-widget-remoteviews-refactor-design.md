...
- widget Provider 接收 HINT_WORD_NEXT 后，对全部实例发送完整 RemoteViews，末尾附加一次 `RemoteViews.showNext()`，再用真实 URI/keyword 打开 App。不带 `setDisplayedChild(0)`，不读取或维护集合 position；主 App 不负责推进。局部更新合并会忽略 showNext，不能使用。
- 按 2026-09-14 后续要求删除中转 Activity 与专用主题。Main 的 `onCreate/onNewIntent` 接收初始/新请求，消费后清除参数并保存标记；主页面保留在 App 任务中，冷启动沿用 App 主题。
- 固定 RoutePath、按钮映射、ARouter 异步排序全部迁入 app 模块。widget 仅构造指向主入口的 PendingIntent，不再依赖 ARouter。升级时重绑所有已有组件的点击，不额外查询数据库。详见 [主 App 入口方案](2026-09-14-widget-main-entry.md)。
...
- 六个入口在真实进程冷启、新建 Main、复用 Main 时均使用本次动作/暗词；另行验收荣耀真实桌面动画期间的点击，不能用直接发送 PendingIntent 的测试替代。
